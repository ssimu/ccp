# ccp — Claude Code / Codex 계정 프로필 전환기
...
`install.sh --statusline` 은 `statusline.sh` 를 `~/.claude/statusline-command.sh` 로 링크하고 `~/.claude/settings.json` 의
`statusLine` 을 그쪽으로 잡는다. Claude Code 하단에 `team:me@example.com | Opus | ~/proj | Context: 12% | git: main` 처럼 뜬다.
어느 계정으로 대화 중인지 늘 보여야 하기 때문이다. `jq` 가 필요하다. 이미 다른 statusline 을 쓰고 있으면 이전 값을 출력하고 덮는다.
