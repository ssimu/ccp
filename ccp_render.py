...
from ccp_i18n import t, left as i18n_left, weekday   # 표시 문구는 전부 ccp_i18n.py 에서
...
mlabel=next((r['mn'] for r in rows if r['mn']),t('model'))
WEEKLY=t('weekly'); SESSION=t('session')
for r in rows:
    if r['st']!='ok':
        r['grp']=3; r['slack']=-1; r['rate']=-1; r['remain']=None; r['bind']=WEEKLY; r['bindp']=None; r['flag']=False; continue
    ...
    r['bind']=mlabel if mrem<100-w else WEEKLY
...
GHEAD={0:(t('g0'),'32'), 1:(t('g1'),'1;33'), 2:(t('g2'),'1;31'), 3:(t('g3'),'2')}
...
def rec_summary():
    ...
    for tool in HASTOOL:
        r=RECS.get(tool)
        parts.append(f'{TOOLNAME[tool]} → '+(f'{r["i"]}) {r["name"]}' if r else t('rec_none')))
    return C(cut(t('rec_summary',parts=' · '.join(parts)),COLS-1),'1;32')
...
def note_of(r,short,full=False):
    ...
    if g==0:
        ...
        which=r['bind']
        why=t('note_left',which=which,n=r["remain"])
        if which==mlabel and full: why+=t('note_weekly_is',w=100-(r["w"] or 0))   # 카드에서만 — 표는 칸이 좁다
        if r['remain']<=10 and r['remain']>0: why+=t('note_almost')
        why+=t('note_reset_in',t=left(r['wm']) or t('seven_days'))
    if is_rec: note=t('note_rec')+' · '+why if g==0 else t('note_rec')
    elif g==0: note=why
    elif g==1: note=t('note_sess_out') + ('' if short else t('note_frees_in',t=left(r['sm']) or '?'))
    elif g==2: note=t('note_week_out') + ('' if short else t('note_frees_in',t=left(r['wm']) or '?'))
    else:      note='— '+(t('nologin') if r['st']=='nologin' else t('query_fail'))
    if g==0 and r['mp'] is not None and r['mp']>=100:
        note+=t('note_model_out',m=mlabel)
    if r['extra']: note+=' · '+r['extra']
    if cur and r['name']==cur and r['tool']==cur_tool: note+=t('note_current')
    return note
...
def render_row(r,hl):
    ...
    if LAYOUT=='two':
        ...
        l2=[(' '*8+WEEKLY+' ',base)]+usage_segs(r,base,dim,False)
        # 2줄째 마지막 칸이 잘리지 않게 세션 라벨을 끼운다
        l2=[l2[0],l2[1],l2[2],(' · '+SESSION+' ',base),l2[3],l2[4],(' · '+mlabel+' ',base),l2[5]]
...
def header_lines():
    if LAYOUT=='two':
        out=[t('tbl_legend1_narrow'),
             (t('tbl_legend2_clock',m=mlabel) if SHOWCLOCK else t('tbl_legend2_noclock',m=mlabel))]
    else:
        out=[t('tbl_legend1'), t('tbl_legend2',m=mlabel)]
    out=[C(cut(x,COLS-1),'2') for x in out]
    if LAYOUT!='two':
        # 칸 구조를 그대로 따른다: [막대 ](wide만) + %(4) + ' '+리셋(16)
        barw=(BARW+1 if LAYOUT=='wide' else 0)
        lab=(t('col_weekly_used'),t('col_sess_used')) if LAYOUT=='wide' else (WEEKLY,SESSION)
        out.append(C(' '*LEAD
            +pad(lab[0],barw+PCTW)+pad(t('col_reset'),1+RSTW)
            +pad(lab[1],barw+PCTW)+pad(t('col_reset'),1+RSTW)
            +pad(mlabel,6),'2'))
    return out
...
def build_table(hl_idx):
    ...
    if rec is None:
        ...
        if soon is not None:
            ...
            kind=SESSION if soon['grp']==1 else WEEKLY
            lines.append(C(cut(t('warn_none_soon',i=soon["i"],name=soon["name"],kind=kind,when=when),COLS-1),'1;33'))
        else:
            lines.append(C(t('warn_none'),'1;31'))
    return lines
...
def when(m,ts):
    ...
    d,tm=ts.split(' '); mo,da=int(d[:2]),int(d[3:])
    now=_dt.datetime.now()
    yr=now.year+1 if mo<now.month-6 else now.year
    try: dd=_dt.date(yr,mo,da)
    except ValueError: return tm
    delta=(dd-now.date()).days
    if delta==0: return t('today',t=tm)
    if delta==1: return t('tomorrow',t=tm)
    return t('when_date',mo=mo,da=da,wd=weekday(dd.weekday()),t=tm)
def reset_desc(m,ts,pct,room=10**6):
    if m is None: return t('rd_new') if pct==0 else '-'
    if m<=0: return t('rd_soon')
    for cand in (t('rd_full',when=when(m,ts),left=left(m)), t('rd_mid',when=when(m,ts),left=left(m)), t('rd_short',left=left(m))):
...
def card(r,hl,fold=False):
    ...
    trip=[(LABELS[0],r['w'],tw,reset_desc(r['wm'],r['wt'],r['w'],room)),
          ...
          (LABELS[2],r['mp'],tw,(t('rd_with_weekly')+(t('rd_binds_first') if r['flag'] else '')) if r['mp'] is not None else '-')]
    if r['tool']!='claude':
        # codex 는 모델별 한도가 없고, 요금제에 따라 5시간 창도 없다 — 없는 줄은 아예 안 그린다.
        trip=[x for x in trip[:2] if x[1] is not None] or trip[:1]
    for lab,pct,frac,desc in trip:
        left_=ind+pad(lab,LABW)+' '
        line=C(left_,base)+gbar(pct,GBARW,frac)+' '+C(pctstr(pct),hot(pct,base,dim))+'   '+C(cut(desc,max(0,room)),base if not dim else '2')
        out.append(line)
    return out
...
def build_graph(hl_idx,terse=False,fold=False,tight=False):
    ...
    leg=(t('gr_legend_wide',m=mlabel) if COLS>=100 else t('gr_legend_narrow',m=mlabel))
    if terse:
        lines=['',C(cut(t('gr_legend_terse',m=mlabel,tick=TICK),COLS-1),'2')]
    else:
        lines=['',C(cut(leg,COLS-1),'2'),
               C(cut(t('gr_legend_rec',m=mlabel),COLS-1),'2'),
               C(cut(t('gr_legend_tick',tick=TICK),COLS-1),'2')]
        if 'codex' in HASTOOL:
            lines.append(C(cut(t('gr_legend_codex'),COLS-1),'2'))
    global LEGN
    LEGN=len(lines)
    seen=set(); curtool=None
    for k,r in enumerate(rows):
    ...
    if rec is None:
        ...
        if soon is not None:
            ...
            kind=SESSION if soon['grp']==1 else WEEKLY
            lines.append(C(cut(t('warn_none_soon',i=soon["i"],name=soon["name"],kind=kind,when=when_),COLS-1),'1;33'))
        else:
            lines.append(C(t('warn_none'),'1;31'))
    return lines
...
def timeline_lines():
    ...
    ticks=[]; m=first; k=1
    while m<SPAN:
        ticks.append((pos(m),weekday(now.weekday()+k))); m+=1440; k+=1
    hdr=[' ']*TLW
    for p_,lab in ticks:
    ...
    for r in data:
        ...
        if r['wm'] is None:
            line=C(' '*4+pad(cut(r['tl'],NAMEW),NAMEW)+'  ',base)+C(''.join(axis),'2')+'  '+C(t('rd_new') if w==0 else '-',base)
        else:
            p_=pos(r['wm']); axis[p_]='●'
            ...
            desc=t('rd_soon')
            if r['wm']>0:
                cands=((t('tl_weekly_model',wh=wh,w=w,m=mlabel,mp=r["mp"]), f'{wh} · {mlabel} {r["mp"]}% ◀', wh)
                       if r['flag'] else (t('tl_weekly_used',wh=wh,w=w), f'{wh} · {w}%', wh))
                for cand in cands:
                    desc=cand
